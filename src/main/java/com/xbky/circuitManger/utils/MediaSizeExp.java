...
public class MediaSizeExp  extends Size2DSyntax implements PrintRequestAttribute {
     ...
     */
    public MediaSizeExp(float x, float y,int units) {
        super (x, y, units);
        if (x > y) {
     ...
     *   {@code units < 1} or {@code x > y}.
     */
    public MediaSizeExp(int x, int y,int units) {
        super (x, y, units);
        sizeVector.add(this);
    }
     ...
     *   {@code units < 1} or {@code x > y}.
     */
    public MediaSizeExp(float x, float y,int units, MediaSizeName media) {
        super (x, y, units);
        if (x > y) {
            throw new IllegalArgumentException("X dimension > Y dimension");
        }
     ...
     *   {@code units < 1} or {@code x > y}.
     */
    public MediaSizeExp(int x, int y,int units, MediaSizeName media) {
        super (x, y, units);
        if (media != null && mediaMap.get(media) == null) {
            mediaName = media;
            mediaMap.put(mediaName, this);
