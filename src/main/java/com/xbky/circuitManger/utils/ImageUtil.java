...
/*        BufferedImage image = ImageUtil.getImageUrl("WX210907-234323 维修完毕");
    ...
    public static void printToLabel(String content){
        qrCodePrint(content,45,9,500,100);
    }
    public static void qrCodePrint(String content, int pageWidth, int pageHeight, int showWidth, int showHeight) {
        // 通俗理解就是书、文档
        Book book = new Book();
        // 设置成竖打
        ...
        book.append((graphics, pageFormat, pageIndex) -> {//通过一个匿名内部内实现Printable接口，不懂的自行查看jdk8的新特性
            ...
            graphics.drawImage(getImageUrl(content),0,0,showWidth,showHeight,null);
